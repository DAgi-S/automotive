        ...
        $.get('ajax/get_car_details.php', { car_id: carId })
            .done(function(response) {
                if (response && response.success && response.data) {
                    ...
                    $('#deleteCarInfo').html(`
                        <strong>${car.car_brand} ${car.car_model}</strong><br>
...
/**
 * Setup form validation functionality
 */
function setupFormValidation() {
    ...
    $('#car_year, #edit_car_year').on('input', function() {
        const year = parseInt($(this).val());
        const currentYear = new Date().getFullYear();
        
        if (year < 1900 || year > currentYear + 1) {
            $(this).addClass('is-invalid').removeClass('is-valid');
            $(this).next('.invalid-feedback').text(`Year must be between 1900 and ${currentYear + 1}`);
        } else {
    ...
    $('#mile_age, #edit_mile_age').on('input', function() {
        const mileage = parseInt($(this).val());
        
        if (mileage < 0) {
            $(this).addClass('is-invalid').removeClass('is-valid');
            $(this).next('.invalid-feedback').text('Mileage cannot be negative');
        } else {
            $(this).addClass('is-valid').removeClass('is-invalid');
        }
    });
    ...
    $('#plate_number, #edit_plate_number').on('input', function() {
        const plateNumber = $(this).val().trim();
        
        if (plateNumber.length < 3) {
            $(this).addClass('is-invalid').removeClass('is-valid');
            $(this).next('.invalid-feedback').text('Plate number must be at least 3 characters');
        } else {
            $(this).addClass('is-valid').removeClass('is-invalid');
        }
    });
